...
// .wrangler/tmp/bundle-788FS5/checked-fetch.js
...
// .wrangler/tmp/bundle-788FS5/strip-cf-connecting-ip-header.js
...
var UserState = class {
  ...
  async saveSudokuSession(session) {
    await this.state.storage.put(this.sudokuStorageKey(session.id), session);
  }
  async loadSudokuSession(sessionId) {
    const stored = await this.state.storage.get(
      this.sudokuStorageKey(sessionId)
    );
    return stored ?? null;
  }
...
// .wrangler/tmp/bundle-788FS5/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-788FS5/middleware-loader.entry.ts
